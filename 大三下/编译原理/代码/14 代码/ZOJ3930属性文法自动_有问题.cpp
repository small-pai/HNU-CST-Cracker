/**
 * ZOJ3930，有问题，懒得管了，样例本身就有错误
 */
#include <bits/stdc++.h>
using namespace std;

/// 原始数据，第1个为开始符号
const string VN_STR = " ETFD";
...
const vector<function<void(symbol_t&, const vsym&)> > AttribFun = {
    nullptr, 
	[](symbol_t&left, const vsym&right){for(auto &t: right)left.value += t.value, left.value += " "; left.value.pop_back();},
	[](symbol_t&left, const vsym&right){for(auto &t: right)left.value += t.value, left.value += " "; left.value.pop_back();},
	[](symbol_t&left, const vsym&right){left.value = right[0].value;},
	[](symbol_t&left, const vsym&right){for(auto &t: right)left.value += t.value, left.value += " "; left.value.pop_back();},
	[](symbol_t&left, const vsym&right){for(auto &t: right)left.value += t.value, left.value += " "; left.value.pop_back();},
	[](symbol_t&left, const vsym&right){left.value = right[0].value;},
	[](symbol_t&left, const vsym&right){for(auto &t: right)left.value += t.value;},
	[](symbol_t&left, const vsym&right){left.value = right[0].value;},
	[](symbol_t&left, const vsym&right){left.value = right[0].value;},
    [](symbol_t&left, const vsym&right){
		string t = "[d" + right[2].value + "]";
		left.value = "("; left.value += t;
		int cnt = string2int(right[0].value);
		for(int i=1;i<cnt;++i) left.value += " + ", left.value += t;
		if(1 == cnt) left.value = left.value.substr(1);
		else left.value += ")"; 
	},
	[](symbol_t&left, const vsym&right){left.value = "[d" + right[1].value + "]";}
};
...
bool lex(){
    ...
    for(int i=0,n=Input.length();i<n;++i){
		if(isdigit(Input[i])){
			...
			Tokens.push_back({V2Idx['i'], v});	
			--i;		
		}else if('+' == Input[i] || '*' == Input[i]){            
			Tokens.push_back({V2Idx[Input[i]], string(1, Input[i])});
		}else if('-' == Input[i] || '/' == Input[i]){
			Tokens.push_back({V2Idx[Input[i]], string(1, Input[i])});
		}else if('(' == Input[i] || ')' == Input[i]){
            Tokens.push_back({V2Idx[Input[i]], string(1, Input[i])});
...
bool parse(){
    ...
    vector<symbol_t> symbolStack(1, {0, ""});
...
int main(){
	...
	while(nofkase--){
		getline(cin, Input);
		assert(proc());
		cout<<(LastSymbol.value + " = [Result]")<<endl;
	}
	return 0;
}
