...
void remove(Bnode** pT,int x){ //在T这个树里面删去值为x的节点
	...
	if(flag){ //如果找到了x，那么我们才进行删除
		...
		//用com指向了要要改变的变量 
		if(fa == NULL) com = pT;
		else if(child->data < fa->data) com = &(fa->l);
		else com = &(fa->r);
		
		if(child->l == NULL && child->r == NULL) *com = NULL;
		else if(child->l != NULL && child->r == NULL) *com = child->l;
		else if(child->l == NULL && child->r != NULL) *com = child->r; 
